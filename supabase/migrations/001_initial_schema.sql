-- ============================================
-- 稻穀烘乾機監控系統 - 資料庫 Schema
-- 表格前綴: rhm_ (ricehouse monitor)
-- ============================================

-- 乾燥機設備清單
CREATE TABLE rhm_dryers (
  id SMALLINT PRIMARY KEY,              -- 設備編號 1-16 (對應 Modbus x=0~F)
  name TEXT NOT NULL DEFAULT '',        -- 自訂名稱 (如 "1號機")
  model_code SMALLINT,                  -- 機種代碼 (Modbus bit 0-3)
  model_name TEXT,                      -- 機種名稱 (PHS-130, NP-60HB, etc.)
...
CREATE TABLE rhm_dryer_status (
  ...
  status_code SMALLINT NOT NULL DEFAULT 0xFF,
